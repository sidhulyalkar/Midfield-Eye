...
async function fetchPilot(source: ReturnType<typeof useDataSource>): Promise<PilotPayload> {
  const manifest = await source.getManifest();
  const path =
    typeof manifest.pilot_path === "string" ? manifest.pilot_path : "pilot/index.json";
  const response = await fetch(source.assetUrl(path), {
    headers: { Accept: "application/json" },
  });
  if (!response.ok) {
    throw new Error(`${response.status} ${response.statusText} while loading the R1 pilot`);
  }
...
export default function PilotPage() {
  ...
  return (
    <div className="pilot-page">
      <section className="pilot-hero">
        <div className="pilot-hero-copy">
          ...
          <div className="pilot-status-row">
            ...
            <span className="pilot-next-gate">Next gate · {nextGate(data)}</span>
          </div>
        </div>
        <aside className="pilot-claim-card" aria-label="Current claim boundary">
          ...
          <p>
            Missing evidence stays missing. Synthetic showcase scenarios never fill a blank
            reliability score or benchmark metric.
          </p>
        </aside>
      </section>
      ...
      <section className="pilot-ladder" aria-labelledby="pilot-ladder-title">
        <header>
          <div>
            ...
            <h2 id="pilot-ladder-title">Five locks between an idea and a claim.</h2>
          </div>
          <p>
            Each rung must be satisfied in order. A failed reliability result is a result, not an
            invitation to tune the threshold.
          </p>
        </header>
        <ol>
          {data.evidence_ladder.map((step, index) => (
            <li className={step.complete ? "is-complete" : "is-pending"} key={step.id}>
              <span className="pilot-step-number">{String(index + 1).padStart(2, "0")}</span>
              <div>
                <strong>{step.label}</strong>
                <p>{step.detail}</p>
              </div>
              <span className="pilot-step-state">{step.complete ? "locked" : "open"}</span>
            </li>
          ))}
        </ol>
      </section>
      ...
      <section className="pilot-design-grid">
        <article className="pilot-panel pilot-sample-panel">
          ...
          <p className="pilot-fine-print">
            These strata are sampling heuristics for diversity. They are never tactical labels and
            never enter the model feature matrix.
          </p>
        </article>
        ...
        <article className="pilot-panel pilot-blinding-panel">
          ...
          <div className="pilot-locks">
            ...
            <div>
              ...
              <p>No rank, brightness, or model score can whisper a preferred option.</p>
            </div>
            <div>
              <span aria-hidden="true">03</span>
              <strong>Causal history only</strong>
              <p>Earlier frames can explain creation. Later observed frames remain invisible.</p>
            </div>
          </div>
      ...
      <section className="pilot-benchmark" aria-labelledby="pilot-benchmark-title">
        <header>
          <div>
          ...
          <p>
            R1 is sequence-held-out Tier A. Provider-held-out replication is intentionally deferred
            until an independent full-tracking Tier B dataset is annotated.
          </p>
        </header>
        {data.benchmark.complete && modelRows.length > 0 ? (
          <div className="pilot-metric-table" role="table" aria-label="R1 benchmark metrics">
          ...
          <div className="pilot-empty-result">
            ...
            <div>
              ...
              <p>
                The interface will populate this table only after double annotation, reliability,
                adjudication, causal-feature review, provider-quality review, and the frozen
                sequence-held-out run all succeed.
              </p>
            </div>
          </div>
        )}
      ...
      <section className="pilot-roadmap" aria-labelledby="pilot-roadmap-title">
        <header>
          ...
          <h2 id="pilot-roadmap-title">Build outward only when the current representation breaks.</h2>
        </header>
        <div className="pilot-roadmap-grid">
          ...
          <article>
            ...
            <p>Freeze the method and repeat on Sportec or another full-tracking source.</p>
          </article>
          <article>
            <span>R3</span>
            <strong>Observation stress test</strong>
            <p>SkillCorner and masking experiments ask what disappears when the pitch disappears.</p>
          </article>
          <article>
            <span>R4</span>
            <strong>Representation upgrade</strong>
            <p>Only then earn temporal graphs, video uncertainty, or egocentric perception.</p>
          </article>
        </div>
      </section>
      ...
      <section className="pilot-guardrails">
        <div>
          ...
          <h2>The fastest path is the one that survives contact with bad results.</h2>
        </div>
        <ul>
