...
test("v1.3 comparison workbench restores a citable state and links 3D to the exact slice", async ({
  ...
  await expect(page.getByText("Teaching intervention, not causal evidence")).toBeVisible();
  await expect(page.getByText(/candidate pass scores are intentionally omitted/u)).toBeVisible();
...
test("v1.3 comparison keeps one-sided evidence categorical in explanatory copy", async ({
  ...
  await expect(page.getByText("Color says direction. Shape says whether a number exists.")).toBeVisible();
  await expect(page.getByText(/Vertical gold rails: retained only in A, no numeric Δ/u)).toBeVisible();
  await expect(page.getByText(/Horizontal blue rails: retained only in B, no numeric Δ/u)).toBeVisible();
