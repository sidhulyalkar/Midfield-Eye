...
def _build_player_atlas(output: Path, catalog) -> list[dict[str, Any]]:
    ...
    for player in catalog.players:
        ...
        profile["scenario_ids"] = [
            ...
            if scenario.player_id == player.id
        ]
    ...
    cohort_summary = {
        cohort: {
            "count": len(
                [player for player in catalog.players if player.cohort == cohort]
            ),
            "featured_count": len(
                [
                    player
                    for player in catalog.players
                    if player.cohort == cohort and player.featured
                ]
            ),
            "archetype_count": len(
                {
...
def build_showcase_bundle(
    ...
    *,
    ...
    for scenario_id in selected:
        ...
        _write_json(
            scenario_dir / "options.json",
            [serialize_action_option(option) for option in options],
        )
        _write_json(
            scenario_dir / "counterfactual_options.json",
        ...
        counterfactual = (
            ...
            else render_counterfactual_uplift(
                key_frame,
                key_options,
                counterfactual_path,
        ...
        body_visual = (
            body_path
            if body_path.exists()
            else render_body_mechanics_lab(
                key_frame,
                key_options,
                body,
                body_path,
        ...
        relational_visual = (
            relational_path
            if relational_path.exists()
            else render_relational_control_lab(
                key_frame,
                key_options,
                relational,
                relational_path,
        ...
        scenario_payload.update(
            {
                ...
                "paths": {
                    ...
                    "counterfactual_options": (
                        f"scenarios/{scenario_id}/counterfactual_options.json"
                    ),
                    ...
                    "body_mechanics": (
                        f"scenarios/{scenario_id}/body_mechanics.json"
                    ),
                    "relational_control": (
                        f"scenarios/{scenario_id}/relational_control.json"
                    ),
                    "tactical_lens": str(tactical.relative_to(output)).replace(
                        "\\", "/"
                    ),
                    "action_menu_timeline": str(
                        timeline.relative_to(output)
                    ).replace("\\", "/"),
                    "style_profile": str(style.relative_to(output)).replace(
                        "\\", "/"
                    ),
                    "counterfactual_uplift": str(
                        counterfactual.relative_to(output)
                    ).replace("\\", "/"),
                    "gaze_lab": str(gaze_visual.relative_to(output)).replace(
                        "\\", "/"
                    ),
                    "body_mechanics_lab": str(
                        body_visual.relative_to(output)
                    ).replace("\\", "/"),
                    "relational_control_lab": str(
                        relational_visual.relative_to(output)
                    ).replace("\\", "/"),
                },
            }
        )
    ...
    manifest = {
        ...
        "description": (
            "Frontend-ready 100-player hypothesis atlas plus source-pinned "
            "empirical evidence, ranked action menus, governed perception "
            "inputs, and the v0.7 decision-microscope contract."
        ),
        ...
        "empirical_path": (
            None if empirical_manifest is None else "empirical/manifest.json"
        ),
        ...
        "evidence_contract": {
            "synthetic_showcases": (
                "illustrative only; never presented as real-player measured "
                "performance"
            ),
            "gaze": (
                "literal gaze claims require observed gaze; pose, motion, and "
                "synthetic sources must remain labeled"
            ),
            "body_weight": (
                "weight-transfer and balance are proxies unless "
                "pose/biomechanical sensors are present"
            ),
            "relational_control": (
                "geometry can show response timing but cannot establish "
                "leadership intent alone"
            ),
            "real_player_analysis": (
                "requires rights-cleared footage or licensed tracking and "
                "explicit provenance"
            ),
            "empirical_layer": (
                "source-pinned real examples are separated from inferred proxies "
                "and synthetic demonstrations"
            ),
            "action_menu_lifecycle": (
                "birth and extinction are retrospective visualization labels, "
                "never future-aware focal-frame features"
            ),
            "counterfactual_action_menu": (
                "earlier-run candidate menus are regenerated from the synthetic "
                "Condition B frame with the authoritative Python AffordanceEngine; "
                "scores remain model-derived, not observed availability or causal effects"
            ),
            "youtube": "embed-only reference lane; no downloading or pixel analysis",
        },
        "frontend_contract": {
            "coordinate_system": (
                "105x68 metres, origin top-left, x toward home attack unless "
                "metadata states otherwise"
            ),
            "score_range": (
                "model-dependent; rank within frame before comparing across providers"
            ),
            "responsive_targets": [
                "1440x900",
                "1920x1080",
