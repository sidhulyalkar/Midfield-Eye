...
def join_selected_outcomes(
    ...
    *,
    ...
    candidate_keys = annotations[
        [*FRAME_COLUMNS, "option_key"]
    ].drop_duplicates()
    for selection in selections.to_dict(orient="records"):
        ...
        allowed = set(
            candidate_keys.loc[frame_mask, "option_key"].astype(str)
        )
    ...
    has_observed_candidate = selected_key.notna() & selected_key.astype(str).str.strip().ne("")
    ...
    joined["selection_join_status"] = has_observed_candidate.map(
        {True: "observed_candidate_selected", False: "no_candidate_selected"}
    )
    return joined
...
def selection_join_summary(dataframe: pd.DataFrame) -> dict[str, Any]:
    ...
    if (selected_counts > 1).any():
        raise ValueError("joined selection output has more than one selected candidate in a frame")
    return {
        "schema_version": "action-menu-selection-join-v1",
        "frames": int(len(selected_counts)),
        "frames_with_selected_candidate": int((selected_counts == 1).sum()),
        "frames_without_selected_candidate": int((selected_counts == 0).sum()),
        "selected_candidate_rate": (
            float((selected_counts == 1).mean()) if len(selected_counts) else 0.0
        ),
    }
