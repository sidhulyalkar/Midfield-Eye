...
## v0.5 perception-atlas guardrails

- The catalog contains exactly 100 study candidates, balanced 50 and 50 across the men's and women's game cohorts.
- Never introduce ordinal rank numbers. The cohort is curated, not a universal leaderboard.
- Never rename `showcase_emphasis` as ratings, scores, or ability.
- Never present synthetic named-player scenarios as measured player performance.
- Every gaze output must preserve `gaze_source` and `gaze_confidence`.
- Head pose, body direction, and movement direction are distinct. Do not silently substitute one for another.
- Weight-transfer, braking, turning, and balance values are proxies unless biomechanics sensors are attached.
- Relational geometry cannot establish leadership, communication, intent, or causality by itself.
- Add tests for any new gaze, body, interaction, or player-atlas field.
- Static bundle and API responses must remain contract-compatible.
- A frontend change is incomplete if it breaks 4K, mobile, keyboard, reduced-motion, or evidence-badge behavior.
